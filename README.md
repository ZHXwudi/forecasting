...
- 指数平滑时间序列模型：用指数加权平均估计近期趋势，并叠加小时和星期的季节性模式，属于经典时间序列预测方法。
- LSTM深度学习模型：优先使用 LSTM 神经网络学习连续电价窗口中的非线性变化；如果部署环境没有 TensorFlow，则自动使用稳定的序列窗口近似模型，避免应用报错。
- 最优加权集成模型：根据历史回测误差自动优化多个基础模型的权重，综合得到最终预测结果。

## Streamlit Cloud 报错说明

如果看到 `StreamlitAPIException`，常见原因是页面控件的默认模型名不在当前预测结果列中。这个版本已经做了修复：

- 回测曲线的默认模型会从当前实际可用模型列中动态选择。
- 未来预测的默认模型会优先选择“最优加权集成模型”，如果不存在则自动选择第一个可用模型。
- Streamlit 缓存增加了版本号，避免云端沿用旧英文模型列名导致页面报错。
